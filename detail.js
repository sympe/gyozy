...
function ViewItem(id){
	...
	$("<div>").appendTo("#tips").append("場所："+json.items[id].group).attr('id','detaillocates');  //detaillocatesをitem0に追加
	if(json.items[id].dates.length==1){ //開催日が一日
		Theday.push(json.items[id].dates[0].split("-"));
		...
		date.push(new Date(Theday[0][0],Theday[0][1]-1,Theday[0][2]));
		ThedayWeek.push(weeks[date[0].getDay()]);
		Theday.push(json.items[id].dates[(json.items[id].dates.length)-1].split("-"));
	...
	$("<div>").appendTo("#tips").append(json.items[id].description).attr('id','detailldescription');  //detaillocatesをitem0に追加
	$("<p>").attr("id",'link').appendTo("#tips"); 				//linkをtipsに追加
	$("<a>").attr('href',json.items[id].link).append($("<span>").append("詳しい情報")).appendTo("#link"); //spanをlinkに追加
	$("<div>").attr('id', 'pushbtn').appendTo('#detailcontents').append($('<ul>').append('<li id=tweetbutton>').append('<li id=fav>'));
	$("<a>").attr('href','http://twitter.com/share?text='+'Gyozyからツイート:'+str+'&url='+json.items[id].link)
	...
	$("<img>").appendTo('#dfav').attr('src', 'favstargray.png').attr('id', 'dfavstar').addClass('favstar').css('height', '35px');
	for(var k=0; k<key.length; k++){
		if(json.items[id].title==key[k]){
